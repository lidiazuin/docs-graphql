...
* `_POP`, which expects a single `Int` value as input
* `_PUSH`, which conforms to the type of input defined in the type definition.
...
Similarly, you can also have multiple array property fields and update them in the same query:
...
You can then pop from both the `tags` and `moreTags` property arrays:
...
They are supported in `update` mutations within these entities:
